...
- 注册表严格要求持续时间为正有限数、`maxStacks` 为 1-64 的安全整数、`refresh` 上限为 1，
  倍率为 0-1000 的有限数；组合倍率和最终有效属性每步检查有限值，溢出时明确失败。
- 武器时间片使用半开区间语义：效果在切片末端到期且冷却恰好同时就绪时，发射事件延后到
  下一时间片，以恢复后的伤害和攻击间隔执行；普通帧的既有冷却边界行为不变。
- 默认内容不注册任何效果；本检查点不加入 UI、特效、伤害类型、抗性、光环、区域效果、食物、
  宝箱、NPC 或其他真实内容。

## CP-M5-TARGETING-01

- `CombatPlayer.facing` 保存最后一次非零移动的单位方向，新局默认为 `(1, 0)`；朝向只提供角色视觉反馈。
- 玩家渲染保持圆形碰撞边界：深色外缘、向 `facing` 微偏的内核与同向内嵌高光弧共同表达面向，不绘制轮廓外的方向几何物。
- 散射弹、斧头和默认弹均选择最近敌人；追踪弹继续在飞行中转向最近敌人。
- `separateEnemies` 在追逐更新后以确定性最小距离修正分开敌人；不变更敌人定义或 D-043 的数量档位。
