...
- 交接状态：CONTENT-01 自动验证、独立审计、用户浏览器验收（用户回复 `pass`）和 Green 关闭验证均为 PASS
...
- M5+ RUNTIME、ENEMY-ARCH、DROP-ARCH、WORLD-OBJECTS、EFFECTS 与 CONTENT-01 Green；`CP-M5-ABILITY-01` 范围关卡待进行
...
（D-041 已记录）：与武器分离的被动能力系统（`AbilityDefinition`、注册表、严格创建入口、
玩家运行时能力实例、升级 offer 获取）及第一批能力（斧头、吸经验、追踪弹）。按
`docs/PIPELINE.md` 阶段 0 产出目标/非目标/受影响模块/自动与浏览器验收项并统一
PLAN、STATUS、HANDOFF、ACCEPTANCE 后，才进入设计与实现。不得直接实现能力系统；
不得实现 NPC、程序化地图、敌人行为钩子、稀有度、局外成长、HUD 新 UI。
