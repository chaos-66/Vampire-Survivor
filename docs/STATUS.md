...
- 当前阶段：M5 基础建设；`CP-M5-CONTENT-01` 武器语义修复全新复审 **PASS**，等待用户重新浏览器验收
...
- M5+ 内容：**RUNTIME、ENEMY-ARCH、DROP-ARCH、WORLD-OBJECTS 与 EFFECTS Green；`CP-M5-CONTENT-01` 武器语义修复复审 PASS，等待用户重新验收；`CP-M5-ABILITY-01` 为后续独立检查点**
...
`CP-M5-CONTENT-01` 武器语义修复（D-037：单件替换 + 可升级）复审已 PASS，唯一下一任务是
用户重新浏览器验收；验收通过后按 `docs/PIPELINE.md` 进行 Green 关闭验证与提交。后续
独立检查点 `CP-M5-ABILITY-01`（斧头/吸经验/追踪弹被动能力）需先走范围关卡。后续纯文档
