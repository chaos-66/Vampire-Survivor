...
自动验收：新单局按世界尺寸创建固定且连续 ID 的静态物体；布局不与初始玩家重叠且保持在
世界内；玩家不能穿过障碍并可沿边滑动；无效或超大移动仍返回有限且位于世界内的位置；
可见查询包含内部、边缘接触和绘制边距，排除屏幕外物体且不修改输入；重新开始重建相同布局；
敌人、投射物、掉落、升级和胜负行为保持不变。

浏览器验收：可见障碍只在进入视口时绘制；玩家从四边和角落均不能穿过障碍且沿边移动正常；
移动、战斗、经验、升级、60 秒胜负和重新开始无回归；控制台无未处理错误。

非目标：不实现世界物体注册表、随机/程序化地图、空间索引、敌人/投射物/掉落与障碍碰撞、
可破坏物、交互、效果、食物、宝箱、NPC 或其他真实内容。
