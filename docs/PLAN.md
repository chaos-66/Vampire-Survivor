...
**当前检查点：`CP-M5-ABILITY-01`（D-041）。** 与武器分离的被动能力系统：静态
`AbilityDefinition`、对象身份注册表、严格创建入口、玩家运行时能力实例（独立于武器
持有槽）及经升级 offer 获取/升级；第一批能力为斧头（额外攻击行为，不占武器槽）、
吸经验（扩大拾取范围或吸附）、追踪弹（生成追踪投射物）。实现已提交（`a5b959d`），
两次复审 FAIL 均为文档一致性并已修复，第三次全新复审 **PASS**，等待浏览器验收。
...
5. `CP-M5-EFFECTS-01` ✅
6. `CP-M5-CONTENT-01` ✅
7. `CP-M5-ABILITY-01`（当前，实现已提交，审计修复已应用，待复审）
8. `CP-M5-NPC-01`

当前检查点：`CP-M5-ABILITY-01`；不得合并 NPC 或提前实现敌人行为钩子、
程序化地图。
